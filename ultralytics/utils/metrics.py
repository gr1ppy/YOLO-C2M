...
def bbox_iou1(
    ...
    CIoU: bool = False,
    SIoU: bool = False,
    WIoU: bool = False,            # <<-- NEW flag added
    eps: float = 1e-7,
) -> torch.Tensor:
    """
    Calculate the Intersection over Union (IoU) between bounding boxes.

    Supports various shapes for `box1` and `box2` as long as the last dimension is 4.
    If xywh=True, inputs are (x, y, w, h). If xywh=False, inputs are (x1, y1, x2, y2).

    Flags:
      - GIoU: return Generalized IoU metric (higher is better)
      - DIoU: return Distance IoU metric (higher is better)
      - CIoU: return Complete IoU metric (higher is better)
      - SIoU: return SIoU-style metric (higher is better)
      - WIoU: return WIoU-style metric (higher is better)
    """
    # Get the coordinates of bounding boxes
    if xywh:  # transform from xywh to xyxy (but keep w/h as provided)
    ...
    # Union Area
    # note: for xywh=True, w1/h1 are widths/heights already; for xywh=False we computed them above
    union = w1 * h1 + w2 * h2 - inter + eps

    # IoU
    iou = inter / union

    # Extended IoU metrics
    # <-- include WIoU in this check (only change to outer conditional besides adding WIoU flag)
    if CIoU or DIoU or GIoU or SIoU or WIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height

        if CIoU or DIoU or SIoU or WIoU:  # Distance, Complete, SIoU or WIoU
            c2 = cw.pow(2) + ch.pow(2) + eps  # convex diagonal squared

            # center distance squared (note averaged centers as in original)
            rho2 = (
                (b2_x1 + b2_x2 - b1_x1 - b1_x2).pow(2) + (b2_y1 + b2_y2 - b1_y1 - b1_y2).pow(2)
            ) / 4

            # NEW: WIoU (weight IoU by exponential of center distance normalized by enclosing-box squared)
            if WIoU:
                # l2_center = rho2, l2_box = c2
                dist = torch.exp(rho2 / c2.detach())
                return dist * iou

            if CIoU:  # CIoU: iou - center_penalty - shape_penalty
                v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + v * alpha)  # CIoU

            if DIoU and not SIoU:
                return iou - rho2 / c2  # DIoU

            if SIoU:
                # Compute centers (already implicitly used for rho2 above)
                ...
                # ---- SIoU implementation aligned with IouLoss._SIoU (converted to "higher is better" metric)
                # angle cost
                # min of absolute dx,dy
                min_abs = torch.stack([dx.abs(), dy.abs()], dim=-1).min(dim=-1)[0]
                angle = torch.asin(min_abs / (rho2.sqrt() + 1e-4))
                angle = torch.sin(2 * angle) - 2  # shape (~[-3,-1])

                # wh_box is the enclosing box (cw, ch)
                wh_box = torch.stack([cw, ch], dim=-1)
                d_center = torch.stack([dx, dy], dim=-1)

                # dist: angle modulation * normalized squared center offsets
                dist_term = angle[..., None] * torch.square(d_center / (wh_box + 1e-7))
                # collapse to scalar cost as in IouLoss: 2 - exp(...) - exp(...)
                distance_cost = 2.0 - torch.exp(dist_term[..., 0]) - torch.exp(dist_term[..., 1])

                # shape cost (w/h)
                # pred_wh and target_wh correspond to w1,h1 and w2,h2 (already available)
                d_shape_w = (w1 - w2).abs()
                d_shape_h = (h1 - h2).abs()
                big_w = torch.maximum(w1, w2)
                big_h = torch.maximum(h1, h2)
                # avoid division by zero
                w_shape = 1.0 - torch.exp(-d_shape_w / (big_w + 1e-7))
                h_shape = 1.0 - torch.exp(-d_shape_h / (big_h + 1e-7))
                theta = 4  # same exponent as in your IouLoss._SIoU
                shape_cost = (w_shape ** theta) + (h_shape ** theta)

                # Convert the IouLoss-style "loss" into the metric (higher better):
                # IouLoss returned: loss = (1 - iou_metric) + (distance_cost + shape_cost)/2
                # So metric = iou_metric - (distance_cost + shape_cost)/2
                return iou - (distance_cost + shape_cost) / 2.0
