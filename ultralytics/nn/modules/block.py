...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, h_sigmoid, h_swish, CoordAtt
...
class GSPPF(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        p2 = self.m2(p1)
        p3 = self.m3(p2)
        cat1 = torch.cat((y1, p1, p2, p3), dim=1)
        out = self.cv2(cat1)
        return out
...
class SeparableConvBlockECA(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, eca_k=3):
        super().__init__()
        self.dw = nn.Conv2d(in_channels, in_channels, kernel_size, stride=stride,
        ...
        self.act = nn.SiLU()
        self.eca = ECA(out_channels, k_size=eca_k)
...
class C2G3EMA(nn.Module):
    def __init__(self, c1: int, c2: int, w: int = 1, e: float = 0.5):
        super().__init__()
        if w == 1:
        ...
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, c2, k=1, s=1, p=None, g=1, d=1, act=self.act)

        self.cg1 = Conv(self.c, c2, k=1, s=1, p=None, g=1, d=1, act=self.act)

        self.path1 = nn.Sequential(
            GhostConv(self.c, self.c, k=3, s=1),
            Conv(self.c, c2, k=1, s=1, p=None, g=1, d=1, act=self.act) 
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1 = self.cv1(x)
        y = x1.split((self.c, self.c), 1)
        y = [y[0], y[1]]
        out_1 = self.path1(y[1])
        out_2 = self.cg1(y[0])


        return self.attn(out_1 + out_2)


class C2G3SE(nn.Module):
    def __init__(self, c1: int, c2: int, w: int = 1, e: float = 0.5):
        super().__init__()
        if w == 1:
            self.act = nn.LeakyReLU()
        ...
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, c2, k=1, s=1, p=None, g=1, d=1, act=self.act)
        self.cv2 = Conv(self.c, c2, k=1, s=1, p=None, g=1, d=1, act=self.act)
...
class C2G3Coord(nn.Module):
    def __init__(self, c1: int, c2: int, w: int = 1, e: float = 0.5):
        ...
        self.attn = CoordAtt(inp=c2, oup=c2)
...
class C3k2G(nn.Module):
    """C3k2: C2f-like CSP block that optionally uses C3k blocks instead of Bottleneck blocks."""
    ...
    def __init__(
        ...
        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of blocks in the internal ModuleList.
            c3k (bool): If True use C3k blocks for m, otherwise use Bottleneck.
            e (float): Expansion ratio to compute hidden channels.
            g (int): Groups for convolutions (passed to Bottleneck/C3k).
            shortcut (bool): Whether to enable shortcut inside Bottleneck/C3k.
        """
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2+n)*self.c, c2, 1)
        # build ModuleList m: either C3k blocks or Bottleneck blocks
        self.m = nn.ModuleList(
            (C3kG(self.c, self.c, 2) if c3k else GhostConv(self.c, self.c, 3, 1, 1))
            for _ in range(n)
        )
...
class C3k2CG2(nn.Module):
    ...
    def __init__(
        ...
        # build ModuleList m: either C3k blocks or Bottleneck blocks
        self.m = nn.ModuleList(
            (C3kCG2(self.c, self.c, 2) if c3k else GhostConv(self.c, self.c, 3, 1, 1))
            for _ in range(n)
        )
...
class C3kG(nn.Module):
    """C3k: CSP Bottleneck module whose internal Bottleneck blocks use kernel size `k`."""
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5):
        ...
        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of Bottleneck blocks inside m.
            shortcut (bool): Whether to use shortcut connections in internal Bottlenecks.
            g (int): Groups for convolutions passed to Bottleneck.
            e (float): Expansion ratio to compute hidden channels.
            k (int): Kernel size used inside each internal Bottleneck (both convs use k).
        """
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        # left and right 1x1 convs that form the CSP split/merge pattern
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        # final conv to restore channels
        self.cv3 = Conv(2*c_, c2, 1)
        # internal sequence of Bottleneck blocks using k for both convs and e=1.0
        self.m = nn.Sequential(*(GhostConv(c_, c_, k=3) for _ in range(n)))
...
class GABlock(nn.Module):
    def __init__(self, dim: int, num_heads: int, mlp_ratio: float = 1.2, area: int = 1):
        ...
        self.mlp = nn.Sequential(GhostConv(dim, mlp_hidden_dim, 1), GhostConv(mlp_hidden_dim, dim, 1, act=False))
